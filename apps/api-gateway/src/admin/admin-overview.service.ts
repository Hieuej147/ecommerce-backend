...
export class AdminOverviewService {
  ...
  async get(metadata: Metadata) {
    const [inventory, orderMetrics, paymentMetrics, products, orders] =
      (await Promise.all([
    ...
    const exceptions = orderRows.filter((order) =>
    ...
    return {
      revenue: {
        amountMinor: revenueAmount,
        currency: orderMetrics.currency || 'VND',
      },
      orderCount,
      averageOrder: {
        amountMinor: orderCount ? Math.round(revenueAmount / orderCount) : 0,
        currency: orderMetrics.currency || 'VND',
      },
      paymentHealth: paymentMetrics.totalCount
        ? Math.round(
            (Number(paymentMetrics.paidCount) /
              Number(paymentMetrics.totalCount)) *
              10000,
          ) / 100
        : 100,
      inventory,
