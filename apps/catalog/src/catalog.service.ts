...
export class CatalogService implements CatalogServiceController {
  ...
  private toProto(product: Prisma.ProductGetPayload<object>): Product {
    ...
    if (
      ...
      product.images.forEach(
        (img, idx) => (imagesMap[`color${idx}`] = String(img)),
      );
    }
    ...
    return {
      ...
      price: {
        amountMinor: Number(product.priceAmountMinor),
  ...
  private requireAdmin(role: string): void {
    if (role !== 'admin')
      this.error('PERMISSION_DENIED', 'Admin role required');
  }
  ...
  async createProduct(
    ...
    const reorderPoint =
      request.reorderPoint && request.reorderPoint > 0
    ...
    try {
      const product = await this.prisma.product.create({
        data: {
          ...
          categorySlug,
          reorderPoint,
          priceAmountMinor: BigInt(request.price.amountMinor),
          currency,
      ...
      if (
        ...
        this.error('ALREADY_EXISTS', 'Slug or SKU already exists');
      throw e;
    }
  }
  ...
  async updateProduct(
    ...
    const currency = request.price?.currency?.trim().toUpperCase() || 'VND';
    if (
      (request.stockQuantity !== undefined && request.stockQuantity < 0) ||
      (request.price &&
        (request.price.amountMinor < 0 || !/^[A-Z]{3}$/.test(currency)))
    ) {
      this.error('INVALID_ARGUMENT', 'Invalid product fields');
    }
    ...
    const product = await this.prisma.product.update({
      ...
      data: {
        ...
        ...(request.description !== undefined
          ? { description: request.description }
          : {}),
        ...(request.stockQuantity !== undefined
          ? { stockQuantity: request.stockQuantity }
          : {}),
        ...(request.price
        ...
        ...(request.images && Object.keys(request.images).length
          ? { images: request.images }
          : {}),
        ...(request.categorySlug
          ? { categorySlug: request.categorySlug.trim() }
          : {}),
        ...(request.reorderPoint !== undefined && request.reorderPoint >= 0
  ...
  async reserveStock(
    ...
    if (
      ...
      request.lines.some((line) => line.quantity <= 0) ||
      new Set(request.lines.map((line) => line.productId)).size !==
        request.lines.length
    )
      this.error('INVALID_ARGUMENT', 'Invalid reservation');
    return this.prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        if (existing.status === ReservationStatus.RELEASED)
          this.error('FAILED_PRECONDITION', 'Reservation already released');
      ...
      for (const line of request.lines) {
        ...
        if (
          ...
          this.error('FAILED_PRECONDITION', 'Insufficient stock');
      }
      for (const line of request.lines)
  ...
  async getInventoryMetrics(
    _request: GetInventoryMetricsRequest,
    actorRole = '',
    ...
    const [total, active, lowStock, outOfStock] = await Promise.all([
      ...
      this.prisma.product.count({
        where: { status: ProductStatus.ACTIVE, stockQuantity: { lte: 20 } },
      }),
      this.prisma.product.count({
        where: { status: ProductStatus.ACTIVE, stockQuantity: 0 },
      }),
    ]);
    return {
      totalProducts: total,
