...
describe('PaymentService', () => {
  ...
  it('returns the existing Checkout Session on retry', async () => {
    ...
    const createSession = jest.spyOn(
      (service as any).stripe.checkout.sessions,
      'create',
    );
    (service as any).stripe.checkout.sessions.retrieve = jest
      .fn()
      .mockResolvedValue({ id: 'cs_1', url: 'https://checkout.test/cs_1' });
  ...
  it('rejects payment reads from another user', async () => {
    ...
    await expect(
      service.getPayment({ paymentId: 'pay_1' }, 'attacker'),
    ).rejects.toMatchObject({ error: { code: 'PERMISSION_DENIED' } });
  });
  ...
  it('verifies and applies a completed Checkout webhook', async () => {
    ...
    (service as any).stripe.webhooks.constructEvent = jest
      ...
      .mockReturnValue({
        ...
        data: {
          object: { metadata: { orderId: 'order_1' }, payment_intent: 'pi_1' },
        },
      } as Stripe.Event);

    const result = await service.processWebhook({
      rawBody: new Uint8Array([1, 2]),
      stripeSignature: 't=1,v1=test',
    });

    expect(result.paymentStatus).toBe(PaymentStatus.PAID);
    expect(orders.markPaymentStatus).toHaveBeenCalledWith(
