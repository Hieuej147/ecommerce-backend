...
export class OrderClient implements OnModuleInit {
  ...
  getOrder(id: string, metadata?: Metadata) {
    return firstValueFrom(
      (this.service.getOrder as any)({ orderId: id }, metadata),
    );
  }
  markPaymentStatus(
    input: Parameters<OrderServiceClient['markPaymentStatus']>[0],
  ) {
    return firstValueFrom(this.service.markPaymentStatus(input));
  }
}
