...
export class PaymentService implements PaymentServiceController {
  ...
  async createCheckoutSession(
    ...
    if (!process.env.STRIPE_SECRET_KEY)
      this.fail('FAILED_PRECONDITION', 'STRIPE_SECRET_KEY is required');
    ...
    if (!request.successUrl || !request.cancelUrl)
      this.fail('INVALID_ARGUMENT', 'success_url and cancel_url are required');
    ...
    if (amount <= 0n)
      this.fail('FAILED_PRECONDITION', 'Order total must be positive');
    const currency = String(
      order.total?.currency || process.env.STRIPE_CURRENCY || 'usd',
    ).toLowerCase();
    if (!/^[a-z]{3}$/.test(currency))
      this.fail('INVALID_ARGUMENT', 'Invalid payment currency');
  ...
  async getPayment(
    ...
    userId = '',
    role = '',
  ): Promise<ProtoPayment> {
    if (!userId) this.fail('UNAUTHENTICATED', 'Actor metadata is required');
    const payment = await this.prisma.payment.findFirst({
      where: {
        OR: [{ id: request.paymentId }, { orderId: request.paymentId }],
      },
    });
    if (!payment) this.fail('NOT_FOUND', 'Payment not found');
    if (role !== 'admin' && payment.userId !== userId)
      this.fail('PERMISSION_DENIED', 'Payment does not belong to actor');
  ...
  async processWebhook(
    ...
    if (!secret)
      this.fail('FAILED_PRECONDITION', 'STRIPE_WEBHOOK_SECRET is required');
    if (!request.rawBody?.length || !request.stripeSignature)
      this.fail(
        'INVALID_ARGUMENT',
        'Webhook body and stripe-signature are required',
      );
    ...
    const existingEvent = await this.prisma.payment.findFirst({
      where: { processedEventId: event.id },
    });
    if (existingEvent)
      return {
    ...
    await this.orders.markPaymentStatus({
      orderId,
